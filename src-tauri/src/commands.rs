...
fn appearance_asset_rel_path(kind: &str) -> Result<&'static str, String> {
    match kind {
        "center_logo" => Ok("assets/center-logo.png"),
        "panel_overlay" => Ok("assets/panel-overlay.png"),
        _ => Err(format!("Unknown image kind: {kind}")),
    }
}

fn resize_to_square_png(img: image::DynamicImage, size: u32) -> image::DynamicImage {
    use image::imageops::FilterType;
    img.resize_to_fill(size, size, FilterType::Lanczos3)
}

#[tauri::command]
pub fn import_appearance_image(
    app_handle: tauri::AppHandle,
    kind: String,
    source_path: String,
) -> Result<String, String> {
    let rel = appearance_asset_rel_path(kind.trim())?;
    let source = source_path.trim().trim_matches('"').trim_matches('\'');
    if source.is_empty() {
    ...
    let resized = resize_to_square_png(img, 256);

    let assets_dir = menu_logic::get_assets_dir(&app_handle);
...
#[tauri::command]
pub fn clear_appearance_image(app_handle: tauri::AppHandle, kind: String) -> Result<(), String> {
    let rel = appearance_asset_rel_path(kind.trim())?;
    if let Some(path) = menu_logic::resolve_asset_path(&app_handle, rel) {
...
#[tauri::command]
pub fn resolve_appearance_asset(app_handle: tauri::AppHandle, rel_path: String) -> Option<String> {
    menu_logic::resolve_asset_path(&app_handle, &rel_path)
